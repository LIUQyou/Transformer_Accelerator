...
profile_program() {
    ...
    # Construct the perf command with specified events
    perf_cmd="perf record -o perf_${program_name}.data -g"
    for event in "${events[@]}"; do
        perf_cmd+=" -e $event"
    done

    # Run the perf command with the program
    echo "Running perf for $program_name"
