...
#define OUTPUT_DIMENSION 32
#define INSIDE_DIMENSION 512


static void block_block_multiplication(float* A, float* B, float* C)
{
    int i, j, k;
...
static void matrix_matrix_multiplication(float* A, float* B, float* C, int rows_matrixA, int inside_dimension, int cols_matrixB)
{
    int i, j, k;
...
int block_block_test() {
    ...
    float* A = new float[Block<int>::BLOCK_WIDTH * Block<int>::BLOCK_LENGTH];
    float* B = new float[Block<int>::BLOCK_LENGTH * Block<int>::BLOCK_WIDTH];
    ...
    // Result block
    Block<int> result;
    float* C = new float[Block<int>::BLOCK_WIDTH * Block<int>::BLOCK_WIDTH];
...
int matrix_matrix_test()
{
    ...
    float* matrixA_ = new float[INPUT_DIMENSION * INSIDE_DIMENSION];
    float* matrixB_ = new float[INSIDE_DIMENSION * OUTPUT_DIMENSION];
    // Initialize blocks with test data
    ...
    float* result_ = new float[INPUT_DIMENSION * OUTPUT_DIMENSION];
    // Initialize result block to zero
    for (int i = 0; i < INPUT_DIMENSION; ++i) {
