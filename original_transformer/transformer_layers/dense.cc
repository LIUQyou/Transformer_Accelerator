...
void Dense::multiplyweight(std::size_t seq_len, uint32_t *input, uint32_t *output) {
    MatrixCompute(seq_len, input, output, weight, input_size_, output_size_);
}
...
void Dense::computeSystolic(std::size_t seq_len, uint32_t *input, uint32_t *output) {
    // input shape [batch_size, input_size_]
    // output shape [batch_size, output_size_]

    // multiplyweight(seq_len, input, output);
    SystolicCompute(seq_len, input, output, weight, input_size_, output_size_);
    // add bias vector here
    if (bias != nullptr) {
        addbias(seq_len, output);
    }
