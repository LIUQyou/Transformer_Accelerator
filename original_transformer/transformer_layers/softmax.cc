...
//         int32_t sum = 0;
//         auto* input_uptr = (uint8_t*) (input + i * (seq_len ));
...
//         }
//         sum = (sum==0) ? sum + 1 : sum;
//         input_uptr = (uint8_t*) (input + i * (seq_len ));
//         for (int j=0; j< seq_len; j++){
...
//     }
    ...
    {
        uint32_t sum = 0;
        uint32_t input_exp[seq_len];
        for (size_t j = 0; j < seq_len; j++)
        {
            // calculate the softmax of the input length
            // input_exp[j] = exp(input[i*seq_len+j]/8);
            input_exp[j] = input[i*seq_len+j]>>10;
            sum += input_exp[j];
        }
        sum = sum/seq_len;
        for (size_t j = 0; j < seq_len; j++)
        {
            input[i*seq_len+j] = (input_exp[j]<<8) / sum;
        }
    }
}
...
void Softmax::post_softmax(uint32_t *input, std::size_t seq_len){
    for (int i =0; i< seq_len; i++){
        auto* input_ptr = (int8_t*) (input + i * (seq_len ));
        for (int j=0; j< seq_len; j++){
